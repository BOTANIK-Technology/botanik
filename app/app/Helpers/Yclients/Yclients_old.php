...
namespace App\Helpers\Yclients\Old;
...
class YclientsApi
{
     ...
     * @throws YclientsException
     */
    public function getI18n($locale = 'ru-RU'): array
    {
     ...
     * @param null $staffId - ID сотрудника. Фильтр по идентификатору сотрудника
     * @param DateTime|null $datetime - дата (в формате iso8601). Фильтр по дате
     *                              бронирования услуги (например '2005-09-09T18:30')
     * @param array|null $serviceIds - ID услуг. Фильтр по списку идентификаторов уже
     *                            выбранных (в рамках одной записи) услуг. Имеет
     *                            смысл если зада фильтр по мастеру и дате.
     * @param array|null $eventIds - ID акций. Фильтр по списку идентификаторов уже выбранных
     ...
     */
    public function putCompany($id, array $fields, string $userToken): array
    {
     ...
     * @param string $title - Название услуги
     * @param string $userToken - Токен для авторизации пользователя
     * @param array|null $fields - Остальные необязательные поля для услуги
     * @return array
     * @throws YclientsException
     * @access public
     * @see http://docs.yclients.apiary.io/#reference/4/0/1
     */
    public function putServices(int $companyId, int $serviceId, int $categoryId, string $title, string $userToken, array $fields = null): array
    {
        $parameters = [
            'category_id' => $categoryId,
            'title' => $title,
     ...
     *
     * @param integer $companyId - ID компании
     * @param integer|null $staffId - ID сотрудника, если нужно работать с конкретным сотрудником
     * @return array
     * @throws YclientsException
     * @access public
     * @see http://docs.yclients.apiary.io/#reference/6//
     */
    public function getStaff(int $companyId, $staffId = null): array
    {
     ...
     * @param DateTime|null $cStartDate
     * @param DateTime|null $cEndDate
     * @param DateTime|null $changedAfter
     * @param DateTime|null $changedBefore
     * @return array
     * @throws YclientsException
     * @access public
     ...
     * @param bool $sendSms
     * @param null $comment
     * @param null $smsRemainHours
     * @param null $emailRemainHours
     * @param null $apiId
     ...
     * @throws YclientsException
     */
    public function putRecord($companyId, $recordId, $userToken, array $fields): array
    {
     ...
     */
    public function getHooks(int $companyId, string $userToken)
    {
