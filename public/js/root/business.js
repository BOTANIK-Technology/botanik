...
function getValues () {
    ...
    let data = {};
    Object.keys(inputs).forEach((name) => {data[name] = inputs[name].value ?? null});
    Object.keys(radios).forEach((name) => {data[name] = radios[name].value ?? null});
    return data;
}

function clearValues () {
    let inputs = getInputs();
    Object.keys(inputs).forEach((name) => {inputs[name].value = ''});
    img.value = '';
}
